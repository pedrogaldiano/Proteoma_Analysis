...
Generate_Graph_CosineSimilarity <- function(imtx, samplesNames, labels) {
  ...
  cosine <- dplyr::mutate(cosine,
                          ...
                          Sample = factor(Sample, levels = unname(samplesNames)),
                          Match = factor(Match, levels = unname(samplesNames)))
