...
for (name in names(matrices)) {
  ...
  # Imputation function
  result <- mice::mice(mtx, method = "rf", m = 1, maxit = 1) #TODO: Switch both to 5!
  result <- as.matrix(mice::complete(result))

  set.seed(123)
  # Save the imputation as .CSV
  dir <- paste0("./data/output/matrices/", "MICE", "/")
  file <- paste("rf", name, "by_matrix", sep = "_")
  write.csv(result, paste0(dir, file))
  rm(result, mtx, dir, file, name)
...
for (name in names(matrices)) {
  ...
  for (group in groups) {
    ...
    # Imputation function
    set.seed(123)
    x <- mice::mice(mtx[, cols], method = "rf", m = 1, maxit = 1) #TODO: Switch both to 5!
  ...
  # Save the imputation as .CSV
  dir <- paste0("./data/output/matrices/", "MICE", "/")
  file <- paste("rf", name, "by_group", sep = "_")
  write.csv(result, paste0(dir, file))
...
for (method in c(
  ...
  for (name in names(matrices)) {
    ...
    # Save the imputation as .CSV
    dir <- paste0("./data/output/matrices/", "MICE", "/")
    file <- paste(method, name, "by_matrix", sep = "_")
    write.csv(result, paste0(dir, file))
    rm(result, mtx, dir, file, name)
...
for (method in c(
  ...
  # Save the imputation as .CSV
  dir <- paste0("./data/output/matrices/", "MICE", "/")
  file <- paste(method, name, "by_group", sep = "_")
  write.csv(result, paste0(dir, file))
  rm(result, mtx, dir, file, x, cols, groups, group, name)
