...
# HeLa + Yeast
diannReport_HY<- Make_DiannReport(samplesDF, organism = "")
HY_diannMatrix <- log2(Make_DiannMatrix(diannReport_HY, header = "Genes") + 1)

# Yeast
diannReport_Yeast <- Make_DiannReport(samplesDF, organism = "YEAST")
Yeast_diannMatrix <- log2(Make_DiannMatrix(diannReport_Yeast, header = "Genes") + 1)

# HeLa
diannReport_HeLa<- Make_DiannReport(samplesDF, organism = "HUMAN")
HeLa_diannMatrix <- log2(Make_DiannMatrix(diannReport_HeLa, header = "Genes") + 1)


matrices <- list(
  HY = HY_diannMatrix,
  Yeast = Yeast_diannMatrix,
  HeLa = HeLa_diannMatrix
)
# Remove unnecessary objects. Do not remove diannReport_HY!
rm(HY_diannMatrix, diannReport_Yeast, Yeast_diannMatrix, diannReport_HeLa, HeLa_diannMatrix)
```
...
for (name in names(matrices)) {
   ...
   groups <- unique(diannReport_HY$condition) #Todo: a nicer way of doing thix
   for (group in groups) {
...
for (name in names(matrices)) {
   ...
   groups <- unique(diannReport_HY$condition)
   for (group in groups) {
     cols <- stringr::str_detect(colnames(mtx), group)
