...
yeast_DiannMatrix <- Make_DiannMatrix(yeast_DiannReport, header = "Genes")
human_DiannMatrix <- Make_DiannMatrix(human_DiannReport, header = "Genes")
...
rm(theme, HL_nProteins, Y_nProteins, H_nProteins, combine_Prot)
```
...
theme <- ggplot2::theme(
  ...
  HL_sparsity <- Plot_Sparsity(diannMatrix, fancyLabels) +
    ggplot2::labs(title = "(A) HeLa + Yeast") + theme
...
combine_sparsity <- patchwork::wrap_plots(HL_sparsity, Y_sparsity, H_sparsity)
...
rm(HL_sparsity, Y_sparsity, H_sparsity, combine_sparsity, theme)
```