...
# Ion Cloud
```{r}
ionCloud <- Generate_IonCloud(diannReport, fancyLabels)
...
kmeans <- Generate_Graph_kmeans(imputedMatrix, numberOfClusters = 2)
pca <- Generate_Graph_PCA(imputedMatrix, numberOfClusters = 2, clusterColors = clusterColor, labels = fancyLabels)

combine <- patchwork::wrap_plots(kmeans, pca, ncol = 2)

ggplot2::ggsave(
  filename = paste0("kMeans_PCA____TESTE", ".png"),
  path = "./data/output/",
  plot = combine,
  width = 24, height = 10,
  units = "in", dpi = 300
