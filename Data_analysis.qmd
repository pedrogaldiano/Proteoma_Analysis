...
cv_mtx <- function(data, condition) {
  ...
  # browser()
    dplyr::select(x, protein, cv, condition)
}
...
write_rds(complete_mtx, "alison_complete_mtx.rds")
```
...
complete_mtx <- read_rds("alison_complete_mtx.rds")
...
#check idea --> Deu certo :D
  # t <- colnames(quantums_limma_mtx) %>%
  #   str_remove_all("_r[0-9]+") %>% 
...
# write_rds(results_Actinonin_15mg_vs_NaCl_QuantUMS, "results_Actinonin_15mg_vs_NaCl_QuantUMS.rds")
