...
csv <- as.matrix(tibble::column_to_rownames(read.csv(csvList[1]), var = "X"))
...
#   name <- stringr::str_remove(basename(dir), ".csv$")
#   methods <- append(methods, setNames(name, dir), name)
# }

x = as.matrix(tibble::column_to_rownames(
  read.csv(names(methods[1])),
  var = "X"
))
