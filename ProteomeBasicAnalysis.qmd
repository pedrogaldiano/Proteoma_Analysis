...
checkLabels <- function() {
  conditions = unique(stringr::str_remove_all(labels, params$regexString))
...
GraphCoefficientOfVariation <- function() {
  ...
  CVs %>%
    dplyr::mutate(condition = factor(
      condition, levels = unique(str_remove(unname(labels), "_r[0-9]+"))
    ))
...
quantums_limma_mtx <- read_rds("quantums_limma_mtx.rds")
...
```{r}
  ...
  completeMTX <- CreateImputationMatrix()
  colnames(completeMTX) <- str_replace_all(colnames(completeMTX), " ", "_")
  
  waldo::compare(complete_mtx, completeMTX)
  
  sum(limmaMTX) #OK
  sum(quantums_limma_mtx) #OK
  
  waldo::compare(design_groups, designGroups) #OK
  waldo::compare(design, dg) #OK

  waldo::compare(contrast_matrix, contrastMatrix) #OK
  
  waldo::compare(fit_quantums, fitQuantums) #OK
  waldo::compare(contrast_fit_quantums, contrastFitQuantums) #OK

  

  limmaMTX <- CreateImputationMatrix() %>% 
  column_to_rownames("protein") %>% 
  ...
  limma::makeContrasts,
  c(samplesToCompare, list(levels = dg))) 
  ...
  alison <- read_rds("results_Actinonin_15mg_vs_NaCl_QuantUMS.rds")
  
  a <- alison[, !names(alison) %in% "condition"]
  
  p <- results_Actinonin_15mg_vs_NaCl_QuantUMS[, !names(results_Actinonin_15mg_vs_NaCl_QuantUMS) %in% "condition"]
  
  waldo::compare(a, p) # OK
  
  
  
  resultQuantUMS <- list()
  significantHiLo <- list()
  
 for (item in names(samplesToCompare)) {
  ...
  rownames_to_column("proteins") %>% 
    dplyr::mutate(
      ...
      condition = str_replace_all(item, "_", " ")
    )
  ...
  plot <- significantHiLo %>% 
    dplyr::group_by(proteins, condition)
  
  plot <- signicantProteins %>% 
    ggplot(aes(x = condition, y = proteins)) +
