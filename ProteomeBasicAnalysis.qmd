...
cat("\n\n1-Global variables were loaded.\n")
```
...
CreateImputationMatrix <- function() {
  ...
  if (!(override) & filesExists) {
    ...
    # janitor::clean_names() 
    #acho que só precisa do clean_names se eu for plotar a matrix de imputação
...
cat("\n\n2-Helper functions were loaded.\n")
```
...
GraphPrecursorRT_mzMapDensity <- function() {
  ...
  cat("\n\n4-Graph ", quote(precursorRT_and_mzMapDensity), ".png were created.\n", sep = "")
...
GraphNumberOfPeptidesAndProteins <- function() {
  ...
  cat("\n5-Graph ", quote(number_of_peptides_and_proteins), ".png were created.\n",
...
GraphSparsityMatrix <- function() {
  ...
  cat("\n6-Graph ", quote(sparsity_matrix), ".png were created.\n", sep = "")
...
GraphQuantUMS <- function() {
  ...
  cat("\n7-Graph ", quote(quantUMS), ".png were created.\n", sep = "")
...
GraphCosSimilarity <- function() {
  ...
  cat("\n8-Graph ", quote(cosineSimilarity), ".png were created.\n", sep = "")
...
# 10) Graph: Principal Component Analysis (PCA) by K-means [Tá diferente, mas não sei pq :c]
```{r}
GraphPCA_Kmeans <- function() {
  ...
  cat("\n\n9-Graph ", quote(pcaPlot_and_kMeansPCA), ".png were created.\n", sep = "")
...
# 11) Graph: Coefficient of Variation [REFACTOR!] ----
```{r}
GraphCoefficientOfVariation <- function() {
  ...
  cat("\n\n10-Graph ", quote(CVplot), ".png were created.\n", sep = "")
}
...
# 12) Graph: Bland-Altman [TODO] ----
```{r}
# TODO:
```

# 13) Graph: Distribution of proteins of interest [TODO] ----
```{r}
# TODO:
```
