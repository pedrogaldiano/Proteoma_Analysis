...
# 2) Global variables
```{r}
# This map should be ordered by the sample-name (code = Sample name r1), this will
...
params <- list(
  ...
  genesToHighlight = c("Il1rap", "Lcn2", "Slc47a1", "Chia", "Mup17", "Mup3",
                        "Umod", "Mup20", "Xpnpep2",  "Nectin4", "Esam", "Alb",
                        "Asah1", "Smpd1", "Smpdl3a", "Apoe", "Apoa1", "Apoa2",
                        "B2m", "Vcam1", "Blvrb", "Orm2", "Orm1", "Mup4", "Obp1a",
                        "Serpina6", "C8g", "Ngp", "Cd36", "Hba", "Hbb-b1",
                        "Hbb-b2","Chil3", "Apcs", "Hp", "Mug1", "Serpina1b",
                        "Serpina3k", "Bcam", "Cdh1", "Cdh11", "Ceacam1", "Cd248",
                        "Cfi", "Fgg", "Fga", "Fgb", "Kng1", "Lum", "Retnla",
                        "Cd200", "Cd44", "Cd48", "Cd36", "Cltrn", "Cp", "Cspg4",
...
CleanDiannReport <- function() {
  ...
  diannReport <- DiannReport() # Not sure if the best approach is to get it inside the function or pass as argument
...
GraphCoefficientOfVariation <- function() {
  ...
  CVs %>%
    dplyr::mutate(condition = factor(
      condition, levels = unique(str_remove(unname(params$labels), "_r[0-9]+"))
    ))
 ...
 for (item in names(params$samplesToCompare)) {
  ...
  result <- resultQuantUMS %>% 
    dplyr::mutate(logFC = round(logFC, 2),
                  ...
                  B = round(B, 2)) %>% 
    left_join(
      diannReport[, c("Protein.Group", "Genes")] %>% 
...
#TODO: Later, would be nice to return the significantHiLo
...
GraphBlandAltman <- function() {
  ...
  mappingID <- read_tsv("id_mapping.tsv") %>% 
  ...
  blandAltmanLimmaPlot <- ggplot(allLimmaResults,
    ...
    geom_label_repel(data = allLimmaResults %>% 
                       dplyr::filter(status != "Not significant" & Gene %in% params$genesToHighlight),
  ...
  #TODO: Put the number of the chunk before the figure name ex: "14-BlandAltmanLimmaPlot.png"
...
# 15) Graph: Distribution of proteins of interest [TODO] ----
```{r}
# TODO:
