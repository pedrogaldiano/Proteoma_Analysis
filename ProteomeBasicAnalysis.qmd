...
GraphBlandAltman <- function() {
  ...
  blandAltmanLimmaPlot <- ggplot(allLimmaResults,
    ...
    geom_label_repel(data = allLimmaResults %>% 
                       dplyr::filter(status != "Not significant" & Gene %in% params$genesToHighlight_blandAltman),
...
GraphProteinsComparison <- function() {
  ...
  imputedMatrix <- CreateImputationMatrix()
  
  mappingID <- read_tsv("id_mapping.tsv") %>% 
    dplyr::mutate(`Gene Names` = str_remove(`Gene Names`, " .*"))
  
  proteinsComparisonPlot <- imputedMatrix %>% 
    left_join(mappingID, by = join_by("protein" == "Entry")) %>%
    dplyr::select(!c("Reviewed", "Entry Name", "Protein names", "Organism", "Length")) %>%
    dplyr::filter(`Gene Names` %in% params$genesToHighlight_proteinsComparison) %>%
