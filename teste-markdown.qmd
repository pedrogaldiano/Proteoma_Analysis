...
RemoveMissingAboveThreshold <- function(data, threshold) {
  ...
  data <- dplyr::mutate(df, prot_miss = apply(df, 1, MeanOfMissing)) %>%
    dplyr::filter(prot_miss <= threshold) %>%
    dplyr::select(-prot_miss)

  cat("Proteins with missingness >", threshold, "were removed from dataset\n")

  return(as.matrix(data))
}

# TODO: o labels é uma variável global. Não faz sentido passar ela como argumento... :c
...
UniqueGenesFilter <- function(diannReport, removeSample, pgScore = 0.75, eqScore = 0.75) {
  result <- diannReport %>%
    dplyr::filter(PG.MaxLFQ.Quality > 0.75 &
                    Empirical.Quality > 0.75 &
                    Run != removeSample) %>%
  ...
  cat("\n\nFilter were applied.\nPG.MaxLFQ.Quality >",pgScore,
      "\nEmpirical.Quality >",eqScore,"\nSample removed:", removeSample,"\n")
  return(result)
}

#TODO: Ficou feião isso, mas a Manu ta falando e eu não consigo pensar :c
UniqueGenesReducer <- function(diannMatrix, threshold) {
  return(RemoveMissingAboveThreshold(diannMatrix, threshold = threshold))
}
...
GraphPrecursorRT_mzMapDensity <- function(runs) {
  diannReport <- DiannReport(runs)
...
GraphPrecursorRT_mzMapDensity(labels)
```

# Graph: Peptides | Proteins ----
```{r}
# Counting the number of unique peptides and proteins per run
GraphNumberOfPeptidesAndProteins <- function(runs, nmaxPeptides, nmaxProteins) {
  diannReport <- DiannReport(runs)
  diannReportByRun <- dplyr::group_by(diannReport, Run, condition) %>%
    dplyr::summarise(
...
x <- GraphNumberOfPeptidesAndProteins(labels, nmaxPeptides = 8200, nmaxProteins = 1900)
```
...
GraphSparsityMatrix <- function(runs, sampleToRemove = "") {
  diannReport <- DiannReport(runs)
  ...
  uniqueGenesReduced <- UniqueGenesReducer(uniqueGenesFiltered, threshold = 0.3)
  sparsityPlotReduced <- SparsityPlot(uniqueGenesReduced)

  
  sparsity_matrix <- (sparsityPlotRaw | sparsityPlotFiltered | sparsityPlotReduced) +
    plot_annotation(tag_levels = "A") &
    theme(plot.tag = element_text(size = 30, face = "bold"))
...
GraphSparsityMatrix(labels, sampleToRemove = "NaCl r1")
