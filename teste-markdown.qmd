...
GenBarGraph <- function(data, x, y, condition, labelX, nmax) {
  
  ggplot(data, aes(y = Run, x = n_peptides, fill = condition)) +
    ...
    geom_text(aes(label = n_peptides,
                  hjust = ifelse(n_peptides > nmax, 1.1, -0.15)),
              color = "black",
              size = 7, nudge_x = -0.5) +
...
peptidesPlot <- GenBarGraph(diannReportByRun,
                             x = n_peptides,
                             y = Run,
                             condition = condition,
                             labelX = "Number of peptides",
                             nmax = 8200)

# Create the proteins graph
proteinsPlot <- GenBarGraph(diannReportByRun,
                            x = n_proteins,
                            y = Run,
                            condition = condition,
                            labelX = "Number of proteins",
                            nmax = 1800)
...
cat("\n\nGraph",quote(number_of_peptides_and_proteins) ,"were created.")

cat("\n\nTotal number of unique  peptides:", n_distinct(diannReport$Stripped.Sequence))
    
cat("\nTotal number of unique proteins:", n_distinct(diannReport$Protein.Ids,"\n"))
