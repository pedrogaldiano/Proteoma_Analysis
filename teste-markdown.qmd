...
# GraphNumberOfPeptidesAndProteins(labels, nmaxPeptides = 8200, nmaxProteins = 1900)
```

# Graph: Sparsity Matrix ----
```{r}
# Genes.MaxLFQ.Unique: matriz de abundância de intensidade normalizada 
#com algoritmo MAX LFQ sendo calculada a partir de peptídeos únicos 
#(peptídeos que dão match apenas com uma proteína)

GraphSparsityMatrix <- function(runs, removeRun = "") {
  diannReport <- DiannReport(runs)
  ...
  uniqueGenesReduced <- RemoveMissingAboveThreshold(uniqueGenesFiltered, threshold = 0.3)
...
# GraphSparsityMatrix(labels, removeRun = "NaCl r1")
```
