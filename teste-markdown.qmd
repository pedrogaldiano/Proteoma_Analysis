...
CleanDiannReport <- function(diannReport) {
...
GraphSparsityMatrix <- function() {
  ...
  # uniqueGenesFiltered <- UniqueGenesFilter(diannReport, sampleToRemove)
  ...
  uniqueGenesCleaned <- CleanDiannReport(diannReport)
...
# Imputation matrix [Quebrou hard] ----

```{r}

CreateImputationMatrix <- function(diannReport,
                                   sampleToRemove = "",
                                   imputationMethod = "rf",
                                   mValue = 1,
                                   maxitValue = 100,
  ...
  stringRDS = paste0("imputedMatrix_",imputationMethod,".rds")
  
  
  #TODO: remover esse if, na vida real se eu mexer na função não quero que ele puxe o .rds
  if (!(file.exists(stringRDS) & file.exists(paste0("result_",stringRDS)))) {
    
    UniqueGenesReduced <- RemoveMissingAboveThreshold(UniqueGenesFilter(diannReport, removeSample = sampleToRemove))
    matrixForImputation <- log2(UniqueGenesReduced + 1) %>%
      as.data.frame() %>%
    ...
    # imputedMatrix <- read_rds("imputedMatrix_rf.rds")
    # resultImputedMatrix <- read_rds("result_imputedMatrix_rf.rds")
    
    result <- mice::complete(imputedMatrix)
    
    write_rds(imputedMatrix, file = stringRDS)
    write_rds(result, file = paste0("result_", stringRDS))
    
    return (result)
  } else {
    return (read_rds(paste0("result_", stringRDS)))
  }
  cat("\n\nImputation matrix were created/loaded")
}

result <- CreateImputationMatrix(DiannReport(labels))

```

# Graph: Cosine Similarity [quebrou tbm] ----

```{r}
GraphCosSimilarity <- function(resultImputedMatrix = "pra facilitar a minha vida", sampleToRemove = "") {
  
  diann <- DiannReport(labels)
  cosineSimilarity <- CreateImputationMatrix(
    RemoveMissingAboveThreshold(
      UniqueGenesFilter(diann, removeSample = sampleToRemove))) %>% #isso aqui ficou uma merda, em?
    
    column_to_rownames("protein") %>%  # Pega os nomes das proteínas e converte elas em nome das linhas (literalmente o que ta escrito dhusiahduiaudi)
    as.matrix() %>% 
    lsa::cosine() %>% 
    as.data.frame() %>% 
